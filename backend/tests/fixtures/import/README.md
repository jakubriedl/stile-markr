...
| `broken-unclosed-tag.xml` | Malformed structure | `400` invalid XML message |
| `broken-wrong-root.xml` | Wrong root element | `400` |
| `broken-missing-required.xml` | Missing student-number | `400` |
| `broken-available-zero.xml` | `available=0` divide-by-zero guard | `400` |
| `broken-negative-marks.xml` | Negative obtained | `400` |
| `empty-mcq-results.xml` | Well-formed empty container (IMP-010) | `400` incomplete scan |
| `not-xml.txt` | Garbage body with markr media type | `400` |
| `not-xml.xml` | XML extension with garbage | `400` |
